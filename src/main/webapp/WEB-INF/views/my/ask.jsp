	                                        ...
	                                        <c:if test="${! empty list1}">
	                                            <c:forEach items="${list1}" var="list1">
	                                                <tr>
	                                                    <td>${list1.bdate}</td>
	                                                    <td>${list1.b_index}</td>
	                                                    <td><a id="a-content" href="${pageContext.request.contextPath}/board/qna/${list1.b_index}">${list1.btitle}</a></td>
	                                                    <!-- 제목누르면 해당 글내용으로 이동링크 -->
	                                                    <td>${list1.bhit}</td>
	                                                </tr>
	                                            </c:forEach>
	                                ...
	                                <div class="charity-pagination">
	                                    <ul class="page-numbers">
	                                        <c:if test="${pageMaker.prev}">
	                                            <li class="previous page-umbers"><a href="${pageContext.request.contextPath}/my/ask${pageMaker.makeQuery(pageMaker.startPage - 1)}"><span aria-label="Next"><i class="fa fa-angle-left"></i>Previous Post</span></a>
	                                            </li>
	                                        </c:if>

	                                        <c:forEach begin="${pageMaker.startPage}" end="${pageMaker.endPage}" var="idx">
	                                            <c:out value="${pageMaker.cri.pageNum == idx?'':''}" />
	                                            <li class="page-umbers"><a href="${pageContext.request.contextPath}/my/ask${pageMaker.makeQuery(idx)}">${idx}</a>
	                                            </li>
	                                        </c:forEach>
	                                        <c:if test="${pageMaker.next && pageMaker.endPage > 0}">
	                                            <li class="next page-umbers"><a href="${pageContext.request.contextPath}/my/ask${pageMaker.makeQuery(pageMaker.endPage +1)}"><span aria-label="Next">Next Post<i class="fa fa-angle-right"></i></span></a></li>
	                                        </c:if>
	                                    </ul>
	                                </div>
	                            </c:if>
