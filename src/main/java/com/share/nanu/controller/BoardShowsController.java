...
public class BoardShowsController {
	...
	*/
	...
	/*
	 * public String bsWrite(MultipartHttpServletRequest multiple, BoardVO boardVO,
	 * AttachmentVO attachmentVO, Model model,
	 * 
	 * @AuthenticationPrincipal MemberDetails md) throws Exception {
	 * 
	 * log.info("인증게시판 컨트롤러  -- write() -- 호출");
	 * 
	 * final int THUMNAIL_WIDTH = 264; final int THUMNAIL_HEIGHT = 300; // 336 UUID
	 * uuid = UUID.randomUUID();
	 * 
	 * String loginMember = md.getUsername(); boardVO.setMember_id(loginMember); //
	 * 로그인한 사람 service.writeBoard(boardVO); // 멤버아이디를 이렇게 가져오는게 맞을까? 아니다. 받아올수가 없네.
	 * 로그인한 사용자 정보를 받아와야함. 일단 테스트는 쿼리문에서 써야겠다. // model.addAttribute("getMember_id",
	 * // nbsService.getBoard(boardVO.getMember_id()));
	 * 
	 * log.info("fileUpload"); // 이미지 저장 절대경로 //
	 ...
	 * 
	 * String path =
	 * multiple.getSession().getServletContext().getRealPath("/resources/attachment"
	 * ); log.info("attachment Path : " + path);
	 * 
	 * Date dt = new Date(); SimpleDateFormat sdf = new
	 * SimpleDateFormat("yyyyMMdd"); String datefolder = sdf.format(dt).toString();
	 * log.info("오늘날짜로 생성한 폴더 이름 : " + datefolder);
	 * 
	 * path = path + "\\" + datefolder; log.info("오늘날짜로 생성한 폴더 경로 : " + path);
	 * 
	 * File dir = new File(path); log.info("오늘날짜로 폴더 생성 확인 : " + dir); if
	 * (!dir.isDirectory()) { dir.mkdir(); }
	 * 
	 * List<MultipartFile> mf = multiple.getFiles("file"); if (mf.size() == 1 &&
	 * mf.get(0).getOriginalFilename().equals("")) { log.info("썸네일이 없습니다."); } else
	 * { for (int i = 0; i < mf.size(); i++) { // 파일명 중복 처리
	 * 
	 * // 본래 파일명 String originalfileName = mf.get(i).getOriginalFilename();// 원본이름
	 * String uuidName = uuid + "_" + originalfileName;// 랜덤문자열 파일이름 중복 방지 String
	 * extension = FilenameUtils.getExtension(originalfileName);// 확장자 // 저장 될 파일명
	 ...
	 * 
	 * String thumNailName = "thumNail_" + uuid + "_" + originalfileName; savePath =
	 * path + "\\" + thumNailName; log.info("썸네일 경로 : " + savePath); File thumNail =
	 * new File(path + "\\" + thumNailName);
	 * 
	 * BufferedImage originalImage = ImageIO.read(originFile); BufferedImage
	 * thumNailImage = new BufferedImage(THUMNAIL_WIDTH, THUMNAIL_HEIGHT,
	 * BufferedImage.TYPE_3BYTE_BGR); Graphics2D graphic =
	 * thumNailImage.createGraphics(); graphic.drawImage(originalImage, 0, 0,
	 * THUMNAIL_WIDTH, THUMNAIL_HEIGHT, null);
	 * 
	 * if (extension.equals("jpg")) { ImageIO.write(thumNailImage, "jpg", thumNail);
	 * } else { ImageIO.write(thumNailImage, "png", thumNail); // png만 나옴 }
	 * log.info("썸네일 생성");
	 * 
	 * // 로그 기록을 system.out.println에서 log.info로수정시 로딩속도 향상 그래도 아직 조금 느림
	 * 
	 * attachmentVO.setOriginname(originalfileName); attachmentVO.setPath(savePath);
	 * attachmentVO.setExtension(extension);
	 * attachmentVO.setB_index(service.getBindex(boardVO));
	 * attachmentVO.setUuidName(thumNailName); service.fileUpload(attachmentVO); } }
	 * return "/board_show/yourSupportList"; }
	 */
